      ...
      <div v-if="!volunteerStore.fetching" class="flex justify-center">
        ...
        <div class="flex-1">
          <p class="text-[13px] text-vologray-400 pe-20">
            Lege hier im ersten Schritt, die freiwillige Person mit ein paar
            grundlegenden Daten an. Alle weiteren Details kannst du im nächsten
            Schritt ergänzen.
          </p>
        </div>
        <!-- right column -->
  ...
  methods: {
    validate() {
      ...
      if (
        ...
        this.formData.gender === "--bitte auswählen--"
      )
      ...
      if (
        ...
        !isPossiblePhoneNumber(this.formData.mobile, "DE")
      )
        ...
        try {
          ...
          const mobile = {
            ...
            value: this.formData.mobile
              ? parsePhoneNumber(this.formData.mobile, "DE").number
              : null,
          };
          ...
          await Promise.all([
            this.formData.mobile
              ? this.contactStore.setContact(voloId, mobile)
              : null,
            this.formData.email.trim()
              ? this.contactStore.setContact(voloId, email)
              : null,
          ]);
        } catch (error) {
          console.error(error);
